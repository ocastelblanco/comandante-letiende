...
| **Tareas completadas** | 32 (ver `TODO.md` §3). |
| **CI/CD** | `.github/workflows/deploy-hosting.yml` — push a `main` despliega Hosting, reglas de Firestore y Cloud Functions. **Los PR reciben un canal de vista previa que solo despliega Hosting — nunca `firestore.rules`** (ver gotcha en §7): un cambio de reglas no se puede verificar de punta a punta en preview, solo tras fusionar. Desde la Tarea 32 **ambos jobs ejecutan `npm test -- --watch=false` antes del build**: una suite en rojo bloquea el merge. |
| **Última Sesión** | 2026-09-21 — Tarea 31 (propina editable, medios de pago nuevos) implementada, PR #44. Pendiente verificar en producción, tras el deploy, la edición de propina de un pedido ya creado (no verificable en preview, ver arriba). |
...
- **Fecha:** 2026-09-21
- **Qué se hizo:**
  - Completado el ciclo de la **Tarea 30** (ver detalle en la entrada anterior de esta sección en el historial de sesiones — incidente del PR #42 fusionado sin revisión, revertido y reaplicado en el PR #43).
  - Implementada la **Tarea 31** — propina editable y medios de pago nuevos: `core/models/payment-methods.ts` (patrón de `category-tree.ts`) reemplaza `card`/`cash`/`nequi`/`daviplata` por `datafono`/`qr`/`efectivo`; diálogo de propina (Porcentaje + Valor) antes y después de enviar el pedido, solo mientras `!order.paid`; `OrderService.updateOrderTip()`; `firestore.rules` gana `onlyUpdatesTip()`. PR #44.
  - **Revisión en preview, dos bugs reales encontrados y corregidos en el mismo PR:** el diálogo de propina (hecho con `AlertController`) no mostraba labels visibles y los cambios no se aplicaban al aceptar. Diagnosticado leyendo el código fuente de `@ionic/core` (no adivinado): `AlertInput.label` solo se renderiza para `radio`/`checkbox`; y un botón sin `handler` devuelve los valores anidados bajo `data.values.*`, no en `data.*` — el código leía la ruta equivocada y siempre caía al valor anterior. Corregido reemplazando el diálogo por un overlay propio con signal de visibilidad (mismo patrón que `products.component.ts`), con `ion-label position="stacked"` real. Ambos hallazgos documentados como gotchas en `CLAUDE.md` §7 e indexados aquí en §7.
  - **Segunda vuelta de revisión:** la edición de propina de un pedido *ya creado* (a diferencia de la del pedido nuevo, que sí funcionó) mostraba el cambio y un instante después revertía. Diagnosticado sin poder usar el emulador de Firestore (esta máquina no tiene Java instalado): comparando `firestore.rules` de este PR contra `origin/main`, se confirmó que `onlyUpdatesTip()` solo existe en la rama del PR — y `.github/workflows/deploy-hosting.yml` reveló que el job `preview` **nunca despliega `firestore.rules`**, solo Hosting. El preview corre contra las reglas ya vigentes en producción (sin `onlyUpdatesTip()`), así que la escritura optimista del cliente se revertía al ser rechazada por el servidor. No es un bug de código: es una limitación estructural de no poder verificar cambios de reglas en preview. A pedido explícito del usuario ("vamos a partir de que va a funcionar bien"), se documentó todo (incluida esta limitación, como gotcha nuevo) y se dejó pendiente la verificación real en producción, después del deploy.
  - Documentación actualizada en el mismo PR #44: `tech-specs.md` (§13 recortada a solo la Tarea 33; §4.3 con `PaymentMethod` y `payment-methods.ts`), `TODO.md` (Tarea 31 al historial, cola con solo la Tarea 33), `CLAUDE.md` §7 (dos gotchas nuevos) y este `MEMORY.md` (checklist, ADR-006 implementado por completo, tabla de estado, gotchas indexados, este contexto).
- **Próxima Tarea:** **Tarea 33** — pruebas de `firestore.rules` con el emulador (única tarea restante en la cola). **Antes de arrancarla**, verificar en producción (tras el deploy de la Tarea 31) que la edición de propina de un pedido ya creado funciona de verdad — es el caso que no se pudo probar en preview.
